...
[27](#bsc-to-bsc-call-via-bicn-with-intra-geran-handover-to-amr---efr)
...
[44](#annex-a-example-for-migrating-a-service-to-trfo)
...
**Codec Configuration :** The Codec Configuration of a codes type ,like
...
The transcoders belong logically to the GSM\_BSS. Speech is transported
...
The mobile Core Network from release 4 onwards has a layered
...
For any call transiting the Nb interface, both OoBTC and TFO procedures
...
The PLMNs may alternately be interconnected using SIP rather than BICC
...
1\. When TFO-Negotiation between transcoder A and transcoder B\'
...
Figure 5.1.2-2: Bandwidth and resource usage for BSC to BSC call via
BICN
...
2\. If TFO is supported on the A interface but codec mismatch occurs
...
3\. Often it is not obvious, whether the call terminates in the PSTN or
via TDM in another BICN with TFO capability. Transcoder B\' is therefore
enabled to perform TFO-Negotiation, but this is not successful (in this
example) and will terminate after about 5 seconds.
...
Optionally the BSC has informed the old BTS (and indirectly or directly
...
[If]{.underline} MSC-S A would have sent the full \"Available Codec
...
After a forth handover, where both radio legs are back to FR\_AMR (or
...
We get a remarkable loss in quality, and a substantial DSP load,
...
With these three different AMR-WB Configurations it is guaranteed that
...
\- Reduce bandwidth requirements for A/Gb mode traffic in the packet
...
The general requirement to ensure maximum speech quality that TrFO and
...
b\) increased deployment costs, since the total transcoding capacity is
...
d\) The MGW would need to support the functionality described in
...
\- Change of codec during a call, where one codec is supported in the
...
b\) With the general A-bis architecture, the selection of the radio
...
The media server inserts the ringing tone and the MSC forwards it
...
possibilities of the Release 4 architecture, i.e. it is assumed that the
network supports packet transport and BICC with the split in call
control and bearer according to Rel-4 specifications as well as
TrFO/OoBTC. As in the previous clause the MSC is split to MSCS and MGW.
However it is assumed that the media server is still the same integrated
box, which is connected via ISUP and using TDM transport with G.711.The
message flow might now look as shown in figure A.5.

Figure A.5 Message flow with split architecture and TrFO

The flow A.5 can be considered as a combination of the flows A.3 and A.4
in the previous two clauses
...
\- As AMR is expected at both Iu and Nb interfaces, at this point there
is no need to insert a transcoder between the two.
...
\- Once the IAM message 8 is sent to the media server, the MGW is
informed in the ADDreq message that G.711 will be used on this link.
Therefore the MGW inserts a transcoder. Again, this is part of usual
call handling.
...
In summary, each time a connection request is sent out, there is a need
to consider whether insertion or removal of a transcoder is required.
The decision depends on the codecs used on the link and the link which
will be connected to it. For this purpose link properties are
configured, and the results of a codec negotiation are taken into
account.

For this example service three points in the call flow have been
identified, where a transcoder may be added or removed depending on the
scenario and configuration: after the initial IAM to the VMSC, after the
IAM to the media server and after the release of the connection to the
media server. With the split architecture, these points become the
establishment and removal of the connections. Now, if the same example
service is considered for various scenarios, then the call flows look
analogous. In each case the decision for transcoder insertion or removal
depends on the scenario. This also holds for scenarios not considered in
the previous clauses e.g. for access via A interface instead of Iu (with
obvious differences and adjustments), or e.g. for a scenario where the
A-party VMSC is different from the B-party GMSC.

In the same way similar example services would have their dedicated
points for transcoder insertion at establishment and release of links
...
######### Annex B: Change history
  ...
  **Date**             **TSG \#**   **TSG Doc.**   **CR**   **Rev**   **Cat**   **Subject/Comment**                          **Old**   **New**
  2004-06              SP-24        SP-040445      \-       \-        \-        Presented for approval                       1.4.0     2.0.0
  2004-06              SP-24        SP-040445      \-       \-        \-        Raised to v.6.0.0 after approval at SA\#24   2.0.0     6.0.0
  2004-09              SP-25        SP-040529      0001     \-        \-        Editorials TR 23.977                         6.0.0     6.1.0
  2007-06              \-           \-             \-       \-        \-        Update to Rel-7 version (MCC)                6.1.0     7.0.0
  2008-06              SP-42        \-             \-       \-        \-        Update to Rel-8 version (MCC)                7.0.0     8.0.0
