...
[67](#annex-a-informative-guidelines-for-usage-of-the-criticality-diagnostics-ie)
...
• During normal operation the CN (CBC) initiates all message transfer
...
\- If the *Category* IE is not given in the WRITE-REPLACE message, the
...
![](media/image4.wmf){width="2.4069444444444446in" height="2.53125in"}
...
The sum of the Service Area(s) included in the *Service Areas List* and
...
This message is sent by the RNC to the CN as unsuccessful response to a
KILL message.
...
*Paging ETWS Indicator* IE is used to indicate to the RNC that the
received SABP message contains ETWS contents that need to be sent via
paging message towards the UE.
...
Warning Type IE indicates the types of the disaster. This IE can be used
by the UE to differentiate tye type of alert according to the type of
disaster.
...
*Warning Security Information* IE is set by the CN, transferred to the
UE by the RNC.
...
The *Broadcast Message Content Validity Indicator* IE indicates that the
*Broadcast Message Content* IE does not contain any valid information.

+----------------+----------+-------+----------------+----------------+
| IE/GROUP NAME  | PRESENCE | RANGE | IE Type and    | Semantics      |
|                |          |       | Reference      | Description    |
+----------------+----------+-------+----------------+----------------+
| Broadcast      | M        |       | ENUMERATED     |                |
| Message        |          |       |                |                |
| Content        |          |       | (Broadcast     |                |
| Validity       |          |       | Message        |                |
| Indicator      |          |       | Content not    |                |
|                |          |       | valid)         |                |
+----------------+----------+-------+----------------+----------------+
...
SABP ASN.1 definition conforms with ITU-T Rec. X.680 \[7\] and ITU-T
Rec. X.681 \[8\].

The ASN.1 definition specifies the structure and content of SABP
messages. SABP messages can contain any IEs specified in the object set
...
sending entity shall construct a SABP message according to the PDU
...
If a SABP message that is not constructed as defined above is received,
...
umts-Access (20) modules (3) sabp (3) version1 (1) sabp-PDU-Descriptions
(0)}
...
value SABP-ELEMENTARY-PROCEDURE.&SuccessfulOutcome
({SABP-ELEMENTARY-PROCEDURES}{\@procedureCode})
...
value SABP-ELEMENTARY-PROCEDURE.&UnsuccessfulOutcome
({SABP-ELEMENTARY-PROCEDURES}{\@procedureCode})
...
{Write-Replace-Extensions} } OPTIONAL,
...
{ ID id-Old-Serial-Number CRITICALITY ignore TYPE Old-Serial-Number
PRESENCE optional } \|
...
{ ID id-WarningSecurityInfo CRITICALITY ignore EXTENSION
WarningSecurityInfo PRESENCE optional } \|

{ ID id-Paging-ETWS-Indicator CRITICALITY ignore EXTENSION
Paging-ETWS-Indicator PRESENCE optional } \|
...
protocolExtensions ProtocolExtensionContainer {
{Write-Replace-Complete-Extensions} } OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer {
{Write-Replace-Failure-Extensions} } OPTIONAL,
...
{ ID id-Failure-List CRITICALITY reject TYPE Failure-List PRESENCE
mandatory } \|
...
protocolExtensions ProtocolExtensionContainer {{Kill-Extensions}}
OPTIONAL,
...
{ ID id-Old-Serial-Number CRITICALITY reject TYPE Old-Serial-Number
PRESENCE mandatory } \|
...
protocolExtensions ProtocolExtensionContainer
{{Kill-Complete-Extensions}} OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer
{{Kill-Failure-Extensions}} OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer
{{Load-Query-Complete-Extensions}} OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer
{{Load-Query-Failure-Extensions}} OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer
{{Message-Status-Query-Extensions}} OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer
{{Message-Status-Query-Complete-Extensions}} OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer
{{Message-Status-Query-Failure-Extensions}} OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer {{Reset-Extensions}}
OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer
{{Reset-Complete-Extensions}} OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer
{{Reset-Failure-Extensions}} OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer {{Restart-Extensions}}
OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer {{Failure-Extensions}}
OPTIONAL,
...
protocolExtensions ProtocolExtensionContainer
{{Error-Indication-Extensions}} OPTIONAL,
...
{ ID id-Serial-Number CRITICALITY ignore TYPE Serial-Number PRESENCE
optional } \|
...
Broadcast-Message-Content ::= BIT STRING (SIZE (1..9968))\-- This IE is
sent from the CN to the RNC containing user information i.e.
...
Number-of-Broadcasts-Completed-List ::= SEQUENCE (SIZE (1..maxnoofSAI))
OF
...
iE-Extensions ProtocolExtensionContainer {
{NoOfBroadcastsCompletedListItemIE-ExtIEs} } OPTIONAL,
...
iE-Extensions ProtocolExtensionContainer {
{RadioResourceLoadingListItemIE-ExtIEs} } OPTIONAL,
...
Service-Areas-List ::= SEQUENCE (SIZE (1..maxnoofSAI)) OF
...
umts-Access (20) modules (3) sabp (3) version1 (1) sabp-CommonDataTypes
(3) }
...
criticality SABP-PROTOCOL-EXTENSION.&criticality
({ExtensionSetParam}{\@id}),

extensionValue SABP-PROTOCOL-EXTENSION.&Extension
({ExtensionSetParam}{\@id})
...
An Abstract Syntax Error occurs when the receiving functional SABP
entity:
...
In the SABP messages there is criticality information set for individual
...
transfer syntax, SABP specifies separately if the presence of these
...
SABP-PROTOCOL-IES, SABP-PROTOCOL-IES-PAIR, SABP-PROTOCOL-EXTENSION or
SABP-PRIVATE-IES.
...
for each reported IE/IE group. In the *Information Element Criticality
Diagnostics* IE the *Repetition Number* IE shall be included and in
addition, if the not comprehended IE/IE group is not at message
hierarchy level 1 (top level; see annex A) also the *Message Structure*
...
for each reported IE/IE group. In the *Information Element Criticality
Diagnostics* IE the *Repetition Number* IE shall be included and in
addition, if the missing IE/IE group is not at message hierarchy level 1
(top level; see annex A) also the *Message Structure* IE shall be
included.
...
######## Annex A (informative): Guidelines for Usage of the Criticality Diagnostics IE
...
Note 1. The IEs F, J, and L do not have assigned criticality. The IEs F,
J, and L are consequently realised as the ASN.1 type SEQUENCE OF of
\"ordinary\" ASN.1 type, e.g. INTEGER. On the other hand, the repeatable
IEs with assigned criticality are realised as the ASN.1 type SEQUENCE OF
of an IE object, e.g. ProtocolIE-Container.
...
Assume further more that a received message based on the above tabular
format is according to figure A.1.

![](media/image17.wmf){width="6.001388888888889in"
height="5.854861111111111in"}

Figure A.1: Example of content of a received SABP message based on the
EXAMPLE MESSAGE
...
![](media/image18.wmf){width="6.254166666666666in" height="3.96875in"}

Figure A.2: Example of a received SABP message containing a not
comprehended IE

If there is an error within the instance marked as grey in the IE G in
the IE J shown in the figure A.2, this will be reported within the
*Information Element Criticality Diagnostics* IE within the *Criticality
Diagnostics* IE as follows:

+-------------------------+----------------+-------------------------+
| IE name                 | Value          | Comment                 |
+-------------------------+----------------+-------------------------+
| IE Criticality          | reject         | Criticality for IE on   |
|                         |                | the reported level,     |
|                         |                | i.e. level 4.           |
+-------------------------+----------------+-------------------------+
| IE ID                   | id-G           | IE ID from the reported |
|                         |                | level, i.e. level 4.    |
+-------------------------+----------------+-------------------------+
| Repetition Number       | 11             | Repetition number on    |
|                         |                | the reported level,     |
|                         |                | i.e. level 4.           |
|                         |                |                         |
|                         |                | (Since the IE E (level  |
|                         |                | 2) is the lowest level  |
|                         |                | included in the         |
|                         |                | *Message Structure* IE  |
|                         |                | this is the eleventh    |
|                         |                | occurrence of IE G      |
|                         |                | within the IE E (level  |
|                         |                | 2).                     |
+-------------------------+----------------+-------------------------+
...
| *second repetition*     |                |                         |
+-------------------------+----------------+-------------------------+
| \>IE ID                 | id-E           | IE ID from the lowest   |
|                         |                | level above the         |
|                         |                | reported level, i.e.    |
|                         |                | level 2.                |
+-------------------------+----------------+-------------------------+
| \>Repetition Number     | 3              | Repetition number from  |
|                         |                | the lowest level above  |
|                         |                | the reported level,     |
|                         |                | i.e. level 2.           |
+-------------------------+----------------+-------------------------+

Note 2. The IE J on level 3 cannot be included in the *Message
Structure* IE since they have no criticality of their own.

Note 3. The repetition number of the reported IE indicates the number of
repetitions of IE G received up to the detected erroneous repetition,
counting all occurrences of the IE G below the same instance of the
previous level with assigned criticality (instance 3 of IE E on level
2).

### A.3.2 Example 2

![](media/image19.wmf){width="6.254166666666666in" height="3.96875in"}

Figure A.3: Example of a received SABP message containing a not
comprehended IE

If there is an error within the second instance (marked as grey) in the
sequence (IE L in the tabular format) on level 3 below IE K in the
structure shown in the figure A.3, this will be reported within the
*Information Element Criticality Diagnostics* IE within the *Criticality
Diagnostics* IE as follows:
...
Note 4. The IE L on level 3 cannot be reported individually included in
the *Message Structure* IE since it has no criticality of its own.

### A.3.3 Example 3

![](media/image20.wmf){width="6.267361111111111in" height="3.96875in"}

Figure A.4: Example of a received SABP message containing a not
comprehended IE

If there is an error within the instance marked as grey in the IE G in
the IE H shown in the figure A.4, this will be reported within the
*Information Element Criticality Diagnostics* IE within the *Criticality
Diagnostics* IE as follows:
...
Note 5. The repetition number of level 4 indicates the number of
repetitions of IE G received up to the detected erroneous repetition,
counted below the same instance of the previous level with assigned
criticality (instance 1 of IE H on level 3).

### A.3.4 Example 4

![](media/image21.wmf){width="6.254166666666666in" height="3.96875in"}

Figure A.5: Example of a received SABP message containing a not
comprehended IE

If there is an error within the instance marked as grey in the IE G in
the IE E shown in the figure A.5, this will be reported within the
*Information Element Criticality Diagnostics* IE within the *Criticality
Diagnostics* IE as follows:
...
|                         |                | level, i.e. level 3.    |
+-------------------------+----------------+-------------------------+
| Repetition Number       | 5              | Repetition number on    |
|                         |                | the reported level,     |
|                         |                | i.e. level 3.           |
|                         |                |                         |
|                         |                | (Since the IE E (level  |
...
![](media/image22.wmf){width="6.254166666666666in" height="3.96875in"}

Figure A.6: Example of a received SABP message with a missing IE

If the instance marked as grey in the IE G in the IE E shown in the
figure A.6, is missing this will be reported within the *Information
Element Criticality Diagnostics* IE within the *Criticality Diagnostics*
IE as follows:

+----------------------------+---------+----------------------------+
| IE name                    | Value   | Comment                    |
+----------------------------+---------+----------------------------+
| IE Criticality             | reject  | Criticality for IE on the  |
|                            |         | reported level, i.e. level |
|                            |         | 3.                         |
+----------------------------+---------+----------------------------+
| IE ID                      | id-G    | IE ID from the reported    |
|                            |         | level, i.e. level 3.       |
+----------------------------+---------+----------------------------+
| Repetition Number          | 4       | Repetition number up to    |
|                            |         | the missing IE on the      |
|                            |         | reported level, i.e. level |
|                            |         | 3.                         |
|                            |         |                            |
...
|                            |         | Structure* IE there have   |
|                            |         | been four occurrences of   |
|                            |         | IE G within the IE E       |
|                            |         | (level 2) up to the        |
|                            |         | missing occurrence.        |
+----------------------------+---------+----------------------------+
| Type of Error              | missing |                            |
+----------------------------+---------+----------------------------+
| Message Structure, *first  |         |                            |
| repetition*                |         |                            |
+----------------------------+---------+----------------------------+
| \>IE ID                    | id-B    | IE ID from level 1.        |
+----------------------------+---------+----------------------------+
| Message Structure, *second |         |                            |
| repetition*                |         |                            |
+----------------------------+---------+----------------------------+
| \>IE ID                    | id-E    | IE ID from the lowest      |
|                            |         | level above the reported   |
|                            |         | level, i.e. level 2.       |
+----------------------------+---------+----------------------------+
| \>Repetition Number        | 3       | Repetition number from the |
|                            |         | lowest level above the     |
|                            |         | reported level, i.e. level |
|                            |         | 2.                         |
+----------------------------+---------+----------------------------+

Note 7. The repetition number of the reported IE indicates the number of
repetitions of IE G received up to but not including the missing
occurrence, counting all occurrences of the IE G below the same instance
of the previous level with assigned criticality (instance 3 of IE E on
level 2).
...
ProtocolExtensions ProtocolExtensionContainer
{{ExampleMessage-Extensions}} OPTIONAL,
...
G-List2 ::= SEQUENCE (SIZE (1..3, \...)) OF ProtocolIE-Container {
{G2-IEs} }
...
G-List3 ::= SEQUENCE (SIZE (1..3, \...)) OF ProtocolIE-Container {
{G3-IEs} }
...
G-List4 ::= SEQUENCE (SIZE (1..3, \...)) OF ProtocolIE-Container {
{G4-IEs} }
...
| history  |         |        |          |          |          |
+----------+---------+--------+----------+----------+----------+
| T        | Version | CR     | Tdoc RAN | New      | Subject  |
| SG RAN\# |         |        |          | Version  | /Comment |
+----------+---------+--------+----------+----------+----------+
| RAN\_07  | \-      | \-     | \-       | 3.0.0    | Approved |
|          |         |        |          |          | at TSG   |
|          |         |        |          |          | RAN \#7  |
|          |         |        |          |          | and      |
|          |         |        |          |          | placed   |
...
|          |         |        |          |          | Control  |
+----------+---------+--------+----------+----------+----------+
| RAN\_08  | 3.0.0   | \-     | R        | 3.1.0    | Approved |
|          |         |        | P-000239 |          | at TSG   |
|          |         |        |          |          | RAN \#8  |
+----------+---------+--------+----------+----------+----------+
| RAN\_09  | 3.1.0   | 011    | R        | 3.2.0    | Approved |
|          |         |        | P-000377 |          | at TSG   |
|          |         | 012    |          |          | RAN \#9  |
|          |         |        |          |          |          |
|          |         | 014    |          |          |          |
...
|          |         | 023    |          |          |          |
+----------+---------+--------+----------+----------+----------+
| RAN\_10  | 3.2.0   | 024    | R        | 3.3.0    | Approved |
|          |         |        | P-000616 |          | at TSG   |
|          |         | 025    |          |          | RAN \#10 |
|          |         |        |          |          |          |
|          |         | 026    |          |          |          |
...
|          |         | 029    |          |          |          |
+----------+---------+--------+----------+----------+----------+
| RAN\_11  | 3.3.0   | 030031 | R        | 3.4.0    | Approved |
|          |         |        | P-010114 |          | at TSG   |
|          |         | 032    |          |          | RAN \#11 |
|          |         |        |          |          |          |
|          |         | 033    |          |          |          |
...
+----------+---------+--------+----------+----------+----------+
  ...
  **Date**             **TSG \#**   **TSG Doc.**   **CR**                            **Rev**   **Subject/Comment**                                                                  **Old**   **New**
  ...
  06/2001              12           RP-010377      036,038,040,042,043,044,048,050             Approved at TSG RAN \#12                                                             4.0.0     4.1.0
  09/2001              13           RP-010582      051                               1         SABP criticality                                                                     4.1.0     4.2.0
  09/2001              13           RP-010582      052                               1         Correction to the Error handling of the ERROR INDICATION message                     4.1.0     4.2.0
  ...
  09/2001              13           RP-010582      062                               2         Clarification of the usage of the Number of Broadcasts Requested IE                  4.1.0     4.2.0
  09/2001              13           RP-010582      064                               1         Clarification of the usage of the SABP Reset Procedure                               4.1.0     4.2.0
  09/2001              13           RP-010582      066                               2         Clarification of the usage of the Service Areas List IE within the Reset Procedure   4.1.0     4.2.0
  ...
  12/2001              14           RP-010852      078                               2         CR on 25.419 (R4) Usage of \'\'Number of Broadcasts Completed List\' IE              4.2.0     4.3.0
  ...
  09/2003              21           RP-030445      118                                         Alignment of title and sub-clause text of chapter 10.3.4.2                           5.4.0     5.5.0
