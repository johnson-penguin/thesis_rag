...
[16](#solution-1-dedicated-pdu-for-up-signalling-message-ip)
...
[37](#x.2-network-options-affected-1)
...
Solution Evaluation 6.14 Solution \#x: LTE-EPC UP IP with option 3X/1X
...
5G and LTE can be implemented independently and together in a number of
...
\[2\] 3GPP TS 38.801:\"Study on new radio access technology: Radio
...
As a result, UEs will continue to be vulnerable to potential attack
...
This solution introduces enhancement to the bearer establishment and
...
|      | and CN UP IP      | with UP IP, the   | with UP IP, the   |
...
If performing cryptographic CRC is not desired for all user plane data
...
There is no change required in the MAC, but it may be desirable for a UE
...
This solution assumes that the UE has been upgraded to support UP IP
...
\- One of the \"R\"bits in the PDCP header can be used to indicate the
...
The transfer of the new IE would also have impact on S10 interface
...
At MME change (both at handover and idle mode mobility) a PDN connection
...
Future changes to the UE\'s NR-PDCP firmware to support full rate option
...
Existing GTP-C signalling compatibility concepts are reused to enable
...
requires UP IP is ever used without UP IP.

### 6.16.4 Solution evaluation

The solution has no impact to any nodes but comes at the cost of not
being able to move bearers that require UP IP to EPS. For a UE with UPIP
set to Required for all its PDN connections (and an MME that does not
support "attach without PDN connectivity"), the NG-RAN would keep the UE
connection until it suffers radio link failure and therefore cause extra
interference into adjacent LTE cells. This is an issue of existing
behaviour and not introduced by this solution.
...
Editor\'s Note: give an overview of the solution making sure that you
identify which key issues are being addressed in this solution.

Editor\'s Note: please identify the network elements and interfaces
...
It is concluded that the Rel-15 architectural approach of applying user
plane integrity protection at NR PDCP layer in TS 33.501 is adopted as
the basis for normative work for the following network options:
...
For Key Issue \#6 (UE connected to 5GC indicating support of UP IP over
eUTRA), Solution \#8 (Using existing 5G UE security capability to signal
the UE support of UP IP over eUTRA to 5GC) is adopted for the normative
work.
...
Editor\'s Note: This clause will capture the conclusions on activating
UP IP when UE connects to EPC via eUTRA.

8 Recommendations
=================

Editor\'s Note: This clause will contain any recommendations for further
work
...
|       |       |       |       |       |       | ent** |       |
+-------+-------+-------+-------+-------+-------+-------+-------+
| 20    | SA    | S3-1  | \-    | \-    | \-    | First | 0.0.1 |
| 19-01 | 3\#94 | 90006 |       |       |       | draft |       |
+-------+-------+-------+-------+-------+-------+-------+-------+
| 20    | SA    | S3-1  | \-    | \-    | \-    | Added | 0.1.0 |
| 19-01 | 3\#94 | 90550 |       |       |       | S3-19 |       |
|       |       |       |       |       |       | 0015, |       |
|       |       |       |       |       |       | S3-1  |       |
|       |       |       |       |       |       | 90101 |       |
|       |       |       |       |       |       | and   |       |
...
|       |       |       |       |       |       | 4289, |       |
+-------+-------+-------+-------+-------+-------+-------+-------+
| 20    | SA    | S3-2  | \-    | \-    | \-    | Added | 0.8.0 |
| 20-04 | 98bis | 00804 |       |       |       | S3-20 |       |
|       |       |       |       |       |       | 0611, |       |
|       |       |       |       |       |       | S3-20 |       |
|       |       |       |       |       |       | 0832, |       |
|       |       |       |       |       |       | S3-20 |       |
...
|       |       |       |       |       |       | and   |       |
+-------+-------+-------+-------+-------+-------+-------+-------+
| 20    | SA    | S3-2  | \-    | \-    | \-    | Added | 0.9.0 |
| 20-05 | 3\#99 | 01430 |       |       |       | S3-20 |       |
|       |       |       |       |       |       | 1366, |       |
|       |       |       |       |       |       | S3-2  |       |
|       |       |       |       |       |       | 01427 |       |
|       |       |       |       |       |       | and   |       |
...
|       |       |       |       |       |       | 01421 |       |
+-------+-------+-------+-------+-------+-------+-------+-------+
| 20    | SA\   | SP-2  |       |       |       | MCC   | 1.0.0 |
| 20-06 | #88-e | 00377 |       |       |       | and   |       |
|       |       |       |       |       |       | Edi   |       |
|       |       |       |       |       |       | tHelp |       |
|       |       |       |       |       |       | r     |       |
|       |       |       |       |       |       | eview |       |
...
|       |       |       |       |       |       | roval |       |
+-------+-------+-------+-------+-------+-------+-------+-------+
| 20    | SA\   | S3-2  | \-    | \-    | \-    | Added | 1.1.0 |
| 20-08 | #100e | 02260 |       |       |       | S3-20 |       |
|       |       |       |       |       |       | 2257, |       |
|       |       |       |       |       |       | S3-20 |       |
|       |       |       |       |       |       | 2045, |       |
|       |       |       |       |       |       | S3-20 |       |
