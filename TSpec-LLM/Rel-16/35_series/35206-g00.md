...
[15](#a2.6-the-mix-column-transformation)
...
The block cipher selected is Rijndael \[6\]. This is the algorithm
...
\- It is more likely that **OP** can be kept secret. (If **OP** is
...
When these algorithms are implemented on a USIM, consideration should be
...
Hence if the one-dimensional index of a byte within a block is **n** and
...
![](media/image5.wmf){width="5.708333333333333in" height="2.0625in"}
...
\^ ft\_tab\[1\]\[byte1(x\[(n + 1) & 3\])\] \\

\^ ft\_tab\[2\]\[byte2(x\[(n + 2) & 3\])\] \\

\^ ft\_tab\[3\]\[byte3(x\[(n + 3) & 3\])\] )

\#define f\_round(bo, bi, k) \\

bo\[0\] = f\_rnd(bi, 0) \^ k\[0\]; \\

bo\[1\] = f\_rnd(bi, 1) \^ k\[1\]; \\

bo\[2\] = f\_rnd(bi, 2) \^ k\[2\]; \\

bo\[3\] = f\_rnd(bi, 3) \^ k\[3\]; \\
...
\^ fl\_tab\[1\]\[byte1(x\[(n + 1) & 3\])\] \\

\^ fl\_tab\[2\]\[byte2(x\[(n + 2) & 3\])\] \\

\^ fl\_tab\[3\]\[byte3(x\[(n + 3) & 3\])\] )
...
\*rc=rnd\_con; /\* pointer to the round constant \*/

Ekey\[0\] = u32\_in(key );

Ekey\[1\] = u32\_in(key + 4);

Ekey\[2\] = u32\_in(key + 8);

Ekey\[3\] = u32\_in(key + 12);
...
b0\[1\] = u32\_in(in + 4) \^ \*kp++;

b0\[2\] = u32\_in(in + 8) \^ \*kp++;

b0\[3\] = u32\_in(in + 12) \^ \*kp++;
...
u32\_out(out, lf\_rnd(b1, 0) \^ kp\[0\]);

u32\_out(out + 4, lf\_rnd(b1, 1) \^ kp\[1\]);

u32\_out(out + 8, lf\_rnd(b1, 2) \^ kp\[2\]);

u32\_out(out + 12, lf\_rnd(b1, 3) \^ kp\[3\]);
