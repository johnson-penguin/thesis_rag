...
NOTE: This cancellation is different from the invokeID based cancel
...
A logical CDR is output when a disconnection event is propagated to the
...
be set to indicate \"User busy (17)\".
...
\(1\) A control relationship has been established if the DP was armed as
a TDP‑R. The IM-SSF moves to the State \"Waiting for Instructions\".

The address of the gsmSCF is fetched from the valid CSI. The IM-SSF
provides all available parameters. Otherwise the IM-SSF proceeds with
call handling without CAMEL Service.

The IM-SSF application timer TSSF is set when the IM-SSF sends
...
If the destination gsmSCF is not accessible then the call proceeds
according to the \'default call handling\' parameter in the CSI.

On expiration of TSSF before receiving any operation, the IM-SSF aborts
the interaction with the gsmSCF and the call continues according to the
\'default call handling\' parameter in the CSI.

If the calling party abandons after the sending of \"InitialDP\", then
the IM-SSF aborts the control relationship by means of an abort to TC.
Note that TC will wait until the first response message from the gsmSCF
has been received before it sends an abort to the gsmSCF (see also
clause 12 of TS 29.078 \[11\]).

Generic error handling for the operation related errors are described in
clause 8 and the TC services that are used for reporting operation
errors are described in clause 12 of TS 29.078 \[11\].
...
This operation is used by the gsmSCF to tear down a call at any phase.
This operation may not be sent to an assisting IM-SSF.

#### 9.16.1.1 Parameters 

\- releaseCause:\
A number giving an indication to the IM-SSF about the reason of
releasing this specific call. This may be used by IM-SSF for generating
...
\"CallInformationReport\" or \"ApplyChargingReport\". Possible armed
EDPs are ignored. All connections and resources related to the call are
released.
...
This operation is used to request the IM-SSF to monitor for a
call‑related event (e.g., BCSM events such as busy or no answer), then
...
> shall be reported. gsmSCF will use the option \"sendingSideID\" only.
>
> \- sendingSideID:\
> \
...
>
> \- applicationTimer:\
> This parameter indicates the NoAnswer timer value for the NoAnswer
> event. If the user does not answer the call within the allotted time,
> the IM-SSF reports the event to the gsmSCF. This timer shall be
...
\(2\) The IM-SSF is in either the state \"Waiting for Instructions\" or
the state \"Monitoring\".

> NOTE: In state \"monitoring\" only requests to disarm detection points
...
\(3\) The IM-SSF remains in the same state, unless all EDPs have been
disarmed and no CallInformationReport or ApplyChargingReport has been
requested; in the latter case the IM-SSF moves to the state \"Idle\".
...
This class 2 operation is used by the gsmSCF to refresh the Tssf
application timer, in order to avoid the Tssf time‑out at the IM-SSF.
...
\(2\) The IM-SSF is in the \"Waiting for Instruction\" state or in the
\"Waiting for End of User Interaction\" state or in the \"Waiting for
End of Temporary Connection\" state.
...
######## Annex A (informative): Change history
  ...
  **Date**             **TSG \#**   **TSG Doc.**   **CR**   **Rev**   **Subject/Comment**                                                    **Old**   **New**
  09/2002              CN\#17       NP-020349                         Creation of version 5.0.0                                              2.0.0     5.0.0
  12/2002              CN\#18       NP-020530      001      1         Correction of ASN.1 definition for the InitialDP MediaType parameter   5.0.0     5.1.0
  12/2002              CN\#18       NP-020530      002      1         ASN.1 syntax basic corrections                                         5.0.0     5.1.0
  12/2002              CN\#18       NP-020532      003      1         Correction of ConnectToResource operation procedure for IMS.           5.0.0     5.1.0
  03/2003              CN\#19       NP-030090      004                ASN.1 syntax basic corrections for IMS CAMEL                           5.1.0     5.2.0
  09/2004              CN\#25       NP-040397      006      1         Correction to ERB pre-condition for IM-SSF state                       5.2.0     5.3.0
  12/2004              CN\#26                                         Rel-6 version created after CN\#26                                     5.3.0     6.0.0
  03/2005              CN\#27       NP-050032      048      1         Correction of IETF Reference in TS 29.278                              6.0.0     6.1.0
  12/2005              CT\#30                                         Rel-7 version was created because of ETSI TISPAN references.           6.1.0     7.0.0
  12/2008              CT\#42                                         Upgraded unchanged from Rel-7                                          7.0.0     8.0.0
  2009-12              \-           \-             \-       \-        Update to Rel-9 version (MCC)                                          8.0.0     9.0.0
  2011-03              \-           \-             \-       \-        Update to Rel-10 version (MCC)                                         9.0.0     10.0.0
  2012-09              \-           \-             \-       \-        Update to Rel-11 version (MCC)                                         10.0.0    11.0.0
  2013-06              CT\#60       CP-130308      0050     1         IETF Reference Update                                                  11.0.0    12.0.0
